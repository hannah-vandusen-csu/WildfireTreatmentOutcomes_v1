...
data_subdirectory <-  "matching_outputs/csv"
filename <- "all_pnts_west.csv"
...
analysis_step <- "Predictor" 

# Outcome set in the user input:
outcome <-  wto_outcome 

# Use a column in the dataframe to group the data: 
# In the final model, we conduct variable reduction at a landscape-scale
group_by <- NULL
...
retained_vars = c('class_cod', 'tr_age')

# Should spatial layers be used:
# In final modeling, spatial layers are not used (FALSE)
spatial_layers <- FALSE

# Should recursive feature elimination (rfe) be used for variable reduction:
# In final modeling, no rfe is used (TRUE)
rfe <-  TRUE
...
FIGURE_DIR <- file.path(local, outcome, 'final_dataframes/west/figures')

# CSV output directory
CSV_DIR <- file.path(local, outcome, 'final_dataframes/west/csv')

# Model output directory
MODEL_DIR <- file.path(local, outcome, "final_dataframes/west/models")
