...
simple_extraction_local <- function(local_directory,
  ...
  fires <- read.csv(paste0(local_directory, '/fire_lists/', fire_list_fp, '.csv')) %>%
    tibble() |> filter(!!sym(wto_outcome) == TRUE)
  ...
  if(var_df[var_df$Abbreviation == 'BPS',][column][1,] == 'TRUE'){
    ...
    BPS_csv <- read.csv(paste0(dirname(local_directory), "/global_data/vegetation/bps/LF2016_BPS_200_CONUS/CSV_Data/LF16_BPS_200.csv")) |>
      dplyr::select(c('BPS_CODE', 'BPS_NAME'))
  }
  ...
  local_extraction_ls <- f_list %>% lapply(function(fireID){
    ...
    if(length(list.files(paste0(point_directory), pattern=fireID)) > 0){
      ...
      if('Treatments' %in% var_r_df$Group & var_r_df[var_r_df$Group == 'Treatments', ][column][1, ] == 'TRUE'){
        ...
        tr_rast <- rast(treatments_fp)
        
        # Calculate treatment area based on unique ID 
        pixel_area <- cellSize(tr_rast$uniqueID, unit="m")
        freq_table <- freq(tr_rast$uniqueID)
        area_by_id <- data.frame(ids = freq_table$value, tr_area = freq_table$count * pixel_area[1]$area[1])
        area_by_id <- area_by_id |> mutate(ids = as.character(ids))
        
        if(wto_outcome == "containment"){
          sample_vect_buffer <- buffer(tr_rast, 25)  # buffer containment points by 25 meters
          treatments_df <- terra::extract(tr_rast, sample_vect_buffer, fun=function(x)
              Mode(x, na.rm = T)[1], ID = F, bind = F)
          
        }else{
         
          treatments_df <- terra::extract(tr_rast, sample_vect, method = "simple", ID=F, bind=F)
        }
        ...
        treatments_df <- treatments_df %>%
          ...
          treatments_df <- treatments_df |> mutate(uniqueID = as.character(uniqueID)) |>
            dplyr::left_join(area_by_id, by = c('uniqueID'= 'ids')) |>
      ...
      if(var_df[var_df$Abbreviation == 'BPS',][column][1,] == 'TRUE'){
        ...
        BPS_df <-  as.data.frame(terra::extract(BPS, sample_vect, fun = median, na.rm = T, ID = F, bind = T)) |>
          dplyr::select(!c(names(sample_vect)[! names(sample_vect) %in% unique_identifer ]))  |>
          mutate(BPS_CODE = as.numeric(as.character(BPS_CODE))) |>
          left_join(unique(BPS_csv), by = 'BPS_CODE') |> dplyr::select(!c('BPS_CODE'))
        
        bps_davis <- read.csv(paste0(global_data_directory, '/vegetation/bps/orginal_davis_bps.csv')) |> 
          dplyr::select(c('BpS_name', 'Cluster_name', 'Coarse_cluster'))
      ...
      if (length(gee_files) == 0){
        ...
        gee_df <- lapply(gee_files, read.csv) %>% bind_rows() %>%
          ...
          group_by(point_id) %>% filter(n() == 1) %>% ungroup() %>%
        ...
        # Filter out any untreated that have fall into treatments 
        # That might have been dropped from the analysis
        if (any(colnames(full_df) %in% 'treatment')){
          full_untreated <- full_df %>% filter(class_cod == 0 & treatment == 0)
          full_treated <- full_df %>% filter(class_cod != 0)
          full_df <- bind_rows(full_treated, full_untreated)
        }
